/**
 * CIRRUS HORIZONS - CORE ENGINE
 * Full CRUD, Monte Carlo, and Supabase Sync.
 */
...
// Bulletproof check for Netlify
if (!SUPABASE_ANON_KEY || SUPABASE_ANON_KEY.length < 20) {
    alert("CRITICAL ERROR: Supabase Key Missing. Check your script.js file.");
}
...
function render() {
    ...
    fd.transactions.forEach(t => {
        ...
        div.innerHTML = `
            <span class="${t.type === 'deposit' ? 'badge-in' : 'badge-out'}">${t.type}</span>
            <span class="font-bold text-main">${t.label}</span>
            <span>$${t.amount.toLocaleString()}</span>
            <button class="icon-btn" style="color:var(--rose);" onclick="deleteItem('transactions', ${t.id})"><i data-lucide="trash-2" style="width:14px;"></i></button>
        `;
    ...
    fd.bills.forEach(b => {
        ...
        div.innerHTML = `
            ...
            <button class="icon-btn" style="color:var(--rose);" onclick="deleteItem('bills', ${b.id})"><i data-lucide="trash-2" style="width:14px;"></i></button>
        `;
...
function bindEvents() {
    ...
    document.getElementById('login-btn').addEventListener('click', async () => {
        ...
        if (error) { const e = document.getElementById('auth-error'); e.textContent = error.message; e.style.display = 'block'; }
    });
    ...
    document.querySelectorAll('.decade-btn').forEach(b => {
        b.addEventListener('click', () => {
            ...
            syncToCloud({ starting_decade: Number(b.dataset.decade) });
        });
    });
